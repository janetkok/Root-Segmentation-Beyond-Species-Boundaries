...
class SegBaseModel(nn.Module):
    ...
    def __init__(self, nclass, pretrained_base=True, **kwargs):
        ...
        dilated = True
        self.nclass = nclass
        self.pretrained = resnet34_v1b(pretrained=pretrained_base, dilated=dilated, **kwargs)
...
class ICNet(SegBaseModel):
    ...
    def __init__(self, nclass=19, pretrained_base=True):
        super(ICNet, self).__init__(nclass, pretrained_base=pretrained_base)
...
class _ICHead(nn.Module):
    ...
    def forward(self, x_sub1, x_sub2, x_sub4):
        ...
        outputs.append(F.interpolate(x_24_cls, scale_factor=16, mode='bilinear', align_corners=True))
        # x_cff_12, x_12_cls = self.cff_12(x_sub2, x_sub1)
        x_cff_12, x_12_cls = self.cff_12(x_cff_24, x_sub1)
        outputs.append(F.interpolate(x_12_cls, scale_factor=8, mode='bilinear', align_corners=True))

        up_x2 = F.interpolate(x_cff_12, scale_factor=2, mode='bilinear', align_corners=True)
        up_x2 = self.conv_cls(up_x2)
        outputs.append(F.interpolate(up_x2, scale_factor=4, mode='bilinear', align_corners=True))
